...
public:
    ...
    constexpr int noise2d(int x, int y)
    {
    ...
    constexpr int lerp(int t, int a, int b) { return a + (t * (b - a) >> 12); }
    constexpr int grad(int hash, int x, int y, int z) {
      int h = hash&15,
          u = h<8?x:y,
    ...
    constexpr int grad2d(int hash, int x, int y) {
      int h = hash&15,
          u = h<8?x:y,
          v = h<4?y:0;
      return ((h&1) == 0 ? u : -u) + ((h&2) == 0 ? v : -v);
